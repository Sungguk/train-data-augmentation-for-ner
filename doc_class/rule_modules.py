...
class Replace_ENT():
    ...
    def __init__(self):
        self.LOC_data = load('entity_info/LOC_data(onlyCONLL)')
        self.PER_data = load('entity_info/PER_data(onlyCONLL)')
        self.ORG_data = load('entity_info/ORG_data(onlyCONLL)')
...
class Replace():
    ...
    def generation_via_replace(self, sent_raw, sent_label, alpha):
        ...
        if noun_thr == 0: noun_thr = 1 # 최소값은 1로 유지
        if verb_thr == 0: verb_thr = 1 # 최소값은 1로 유지
        if adj_thr == 0: adj_thr = 1 # 최소값은 1로 유지
        if adv_thr == 0: adv_thr = 1 # 최소값은 1로 유지        
